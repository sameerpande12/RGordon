...
global nextJobID#make sure that you make this 0 at the end of pingServer - this needs to be kept global to be easily accessible by all the processes in the pool
nextJobID=0
global maxJobID#make sure that you make this 0 at the end of pingServer - this needs to be kept global to be easily accessible by all the processes in the pool
maxJobID=0
...
def pingServer():
    ...
    if(response['message']=='JOB'):
        ...
        nextJobID=1#to store the first job yet to be asssigned
        maxJobID=len(response['data'])
        pool = mp.Pool(max(mp.cpu_count(),5))

        for i in range(numParallelJobs):
            r.append(pool.apply_async(runJob,args=[i,response['data']]))
            nextJobID+=1
