...
while i < start + len(urlInfos):
    ...
    if urlInfos[i][1] not in analysisResult.read().splitlines():
        ...
        if urlInfos[i][3] == '':
            ...
            try:
                subprocess.call(["mm-delay " + str(delayTime) + " ./multi-launch.sh '"+urlInfos[i][3]+"' 10"], shell=True, executable='/bin/bash')
